...
import { toast, ToastContainer, Slide, type Id } from 'react-toastify'
import IP from './data/localIp.ts'
import { UserContext, type UserContextType } from './globalContext.tsx'

function RouterComponent() {
  const state = (useContext(UserContext) as UserContextType).state
  return (
    <>
      {state.isLogined ? (
...
function App() {
  ...
  useEffect((): (() => void) => {
    ...
    if (toastId.current === null || !toast.isActive(toastId.current)) {
      switch (status) {
        case 'login':
          toastId.current = toast.success(msg) as Id
          break
        case 'islogined':
          toastId.current = toast.success('已登录') as Id
          break
        case 'tokenError':
          toastId.current = toast.warning(msg) as Id
          break
        case '404':
          toastId.current = toast.error('服务器未响应') as Id
          break
        default:
          break
      }
      ...
      <div className="Main grid grid-flow-row h-screen bg-white relative">
        <nav className="z-50 fixed grid grid-flow-col grid-cols-[1,1fr] auto-rows-fr h-[3.5rem] w-full bg-white text-black content-center ring-1 ring-gray-200 ring-offset-[2px] ring-offset-gray-100 ">
          ...
          <nav className="left-nav grid grid-flow-col place-content-evenly relative min-w-[500px] text-sm font-medium">
            <img
              src={`http://${IP}:5006/static/YanamiStand.png`}
              alt="not found"
              className="hidden xl:block w-8 h-full absolute left-10 animate-bounce "
          ...
          <div className="right-nav grid grid-flow-col grid-cols-1 place-items-center content-center">
            <div className="flex flex-1 justify-center w-full">
              <label
                htmlFor="serach"
                className="hidden md:block w-full max-h-10 max-w-[24rem] h-9 text-black/80 "
              >
                <input
                  id="serach"
                  type="text"
                  className=" group border  rounded-md indent-2 w-full h-full opacity-80 bg-gray-200 hover:opacity-100 focus:opacity-100"
                />
            ...
            <div className="relative place-self-end flex items-center gap-2 px-4">
              ...
              <div className="group">
                <img
                  src={`http://${IP}:5006/static/YanamiMeme.png`}
                  alt="not found"
                  className="min-w-10 min-h-10 group-hover:scale-125 transition group-hover:-translate-x-1 group-hover:translate-y-1 duration-200 ease-in-out h-10 w-10 rounded-full"
                />
