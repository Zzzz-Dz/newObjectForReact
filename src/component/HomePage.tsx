...
function PictrueMap() {
  ...
  useEffect(() => {
    ...
    async function startFetching() {
      try {
        const json = await fetch(`http://${ip}:${port}/getWorks`)
          .then(res => res.json())
          .then(v => {
            const { worksData } = v
            return worksData.map((data, index) => {
    ...
    <div className=" grid min-h-[40rem] grid-cols-4 xl1:grid-cols-5 content-evenly gap-x-5 gap-y-[1.5rem] relative mt-8 ">
      ...
      {pictrueDatas.map((data, index) => {
        ...
        let gridStyle =
          id === 1
            ? {
                span: 'row-span-2 col-span-2 h-[23rem] min-w-[500px] max-w-[580px] xl1:min-w-[490px]',
                imgStyle: ' h-full ',
                absoluteText: '',
                information: 'hidden'
              }
            : {
                span: 'h-[13rem] w-[240px] lg2:w-full max-w-[280px] ',
                imgStyle: 'h-[8.5rem]'
              }
        let gridMargin =
        ...
        if (Object.keys(data).length === 0) {
          return (
            <div
              key={id}
              className={gridStyle.span + gridMargin + ' border  rounded-md '}
            >
        ...
        const worksInformation =
          ...
          '@' +
          Object.values(data).slice(2, -1).join('_')
        const FullURL = encodeURIComponent(worksURL + '/' + worksInformation)
        return (
          <div key={data.id} className={gridStyle.span + gridMargin + ' '}>
            <img
              src={`http://${ip}:${port}/File/${FullURL}`}
              alt={`${data.pictrueName}`}
              className={gridStyle.imgStyle + ' w-full rounded-lg '}
              style={{
            ...
            <h3
              className={
                gridStyle.absoluteText +
                ' pr-7 h-[2.75rem] mt-[0.5rem] tracking-normal font-medium '
              }
              title="123"
            >
              <a href="/">{data.worksName}</a>
            </h3>
